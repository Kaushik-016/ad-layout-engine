      ...
      >
        {layout.placements
          ...
          .map((p) => (
            ...
            >
              <ElementVisual type={p.element.type} content={p.element.content} />
            </div>
          ))}
      </div>
...
function ElementVisual({ type, content }: { type: string; content: string }) {
  switch (type) {
    case 'headline':
    ...
    case 'logo':
      return (
        <div style={{ width: '100%', height: '100%', background: '#ccc', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#888' }}>
          logo
        </div>
      )
    default:
